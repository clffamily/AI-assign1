    ...
    def successors(self, state):
        ...
        for conf in self.configurations:
            if conf[0] != state.conf and state.lasttime_conf == False:
                States.append( ("Set Configuration(" + conf[0] + ")", 0,
...
def h_custom(problem, state):
    ...
    for job in simplified_jobs:
        if job not in state.job_completed:  # Find out the jobs which have not been done yet
            i = 0
            while ( i < len(problem.jobs)):
                if job == problem.jobs[i][0]:
                    cost_tuple = problem.jobs[i][4]
                    hval_max = 0
                    for cost_pair in cost_tuple:
                        if state.time + problem.jobs[i][3] > cost_pair[0]:
                            hval_max = max(hval_max, cost_pair[1]) # Find out the max cost if performing this job late
                    hval = hval + hval_max 
                    break
                i = i + 1
    return hval
